...
import space.doky.simseok.sample2.uiview.R
import space.doky.simseok.sample2.uiview.databinding.FragmentCameraBinding
...
class CameraFragment : Fragment(), ImageClassifierHelper.ClassifierListener {
    ...
    override fun onError(error: String) {
        activity?.runOnUiThread {
            ...
            classificationResultsAdapter.updateResults(null, -1)
            classificationResultsAdapter.notifyDataSetChanged()
        }
    }


    private var categories: MutableList<Category?> = MutableList(1) { null }
    ...
    override fun onResults(
        ...
        activity?.runOnUiThread {
            ...
            classificationResultsAdapter.updateResults(results, imageClassifierHelper.currentModel)
            classificationResultsAdapter.notifyDataSetChanged()
            fragmentCameraBinding.bottomSheetLayout.inferenceTimeVal.text =
                String.format("%d ms", inferenceTime)

            categories = ClassificationResultsAdapter.getSortedCategories(categories, results)

            var categoryName = "none"
            var score = 0.0F

            if (imageClassifierHelper.currentModel == ImageClassifierHelper.MODEL_TEACHABLE) {
                categoryName = MainActivity.Companion.Label.find(categories[0]?.label?.toInt() ?: -1).toString()
            } else {
                categoryName = categories[0]?.label.toString()
            }
            score = categories[0]?.score ?: 0.0F
            message = "category: $categoryName - score: $score}"
            AIAppLog.i("CameraFragment", "onResults", message)
        }
    }
}
